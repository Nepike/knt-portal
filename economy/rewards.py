...
Отсюда: повторный вызов ничего не меняет, звать можно откуда угодно; выплаченное
назад не забирается (иначе «снять и поставить лайк заново» стало бы фермой) —
привести строго к состоянию умеет только `recount_tokens --reset`; достижения лягут
сюда же предикатами на том же состоянии. А чего в состоянии нет, того и не начисляем:
«зашёл сегодня» тут не появится, пока где-то не будет храниться, за сколько дней
уже заплачено.
"""
...
# Стена: клетка — токен, но платим полусотнями, иначе лента операций превратится
# в столбик «+1 пиксель», за которым не видно остального.
WALL_BATCH = 50
...
# Причины, которыми распоряжается пересчёт. Выдать такую руками нельзя: строка без ключа
# зачлась бы как «уже выплачено», и человек молча недополучил бы автоматическую награду.
AUTOMATIC = frozenset({R.WELCOME, R.MATERIAL, R.BOOK, R.REVIEW, R.LIKES, R.DOWNLOAD, R.WALL, R.MODERATION})
