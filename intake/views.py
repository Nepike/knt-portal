...
# Два выхода на полпути: `fail` — не вышло, `release` — остановили руками.
...
@require_POST
def release(request):
    """«Не доделаю» — задание встаёт обратно в очередь прямо сейчас.

    Пекарню остановили с клавиатуры. Это не отказ: печь лекцию по-прежнему можно,
    просто некому в эту минуту, — и `fail` тут был бы неправдой вдвойне. Человек увидел
    бы красное «не обработалась», а из отказа задание вынимают только руками, тогда как
    оно готово печься само. Молчание же оставляет запись «обрабатывается» на весь
    CLAIM_TIMEOUT, пока сайт не сочтёт машину упавшей.

    Недолитую папку снимаем, как и при отказе: следующая выпечка заведёт свою, а эта
    иначе осталась бы в бакете, и найти её потом было бы не по чему.

    Токен взявшего умирает сам: `_job` ищет задание в состоянии «печётся», а мы его
    оттуда только что вынули.
    """
    if refusal := guard(request):
        return refusal
    job = _job(_body(request))
    ...
    job.claimed_by = ""
    job.claimed_at = None
    job.save(update_fields=["status", "prefix", "claimed_by", "claimed_at", "updated"])
    sweep(stale)
    return JsonResponse({"ok": True})
