...
уже на диске. Её замечают студенты, у которых видео начинается через лишний круг
до конца файла и обратно. Проверить это можно только тут.

Модуль **не импортирует Django** и не должен начать — его берёт с собой пекарня
на машину, где проекта нет (см. `intake/spec.py`).
"""
